...
const TASK_COLORS = {
  blue: { hex: '#3B82F6', dot: 'bg-blue-500' },
  green: { hex: '#22C55E', dot: 'bg-green-500' },
  yellow: { hex: '#F59E0B', dot: 'bg-yellow-500' },
  red: { hex: '#EF4444', dot: 'bg-red-500' },
};

const TaskCard = ({ task, onUpdate, onDelete }) => {
  const { isDarkMode } = useAuthStore();
  const color = TASK_COLORS[task.color] || { hex: '#FFFFFF', dot: 'bg-gray-400' };
  const taskBg = isDarkMode
    ? 'bg-gray-700 hover:shadow-indigo-500/30'
    : 'bg-white hover:shadow-indigo-200';
